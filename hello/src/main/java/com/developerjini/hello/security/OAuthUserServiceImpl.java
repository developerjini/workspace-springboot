...
public class OAuthUserServiceImpl extends DefaultOAuth2UserService {
  ...
  public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
    // DefaultOAuth2UserService의 기존 loadUSer를 호출한다. 이 메서드가 user-info-uri를 이용해 사용자
    // 정보를 가져오는 부분이다.
    final OAuth2User oAuth2User = super.loadUser(userRequest);

    try {
      // 디버깅을 돕기 위해 사용자 정보가 어떻게 되는지 로깅한다. 테스팅 시에만 사용해야한다.
      log.info("OAuth2User attributess {} ", new ObjectMapper().writeValueAsString(oAuth2User.getAttributes()));
